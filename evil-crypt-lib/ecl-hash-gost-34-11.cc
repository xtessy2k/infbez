...
static const uint64_t A_MATRIX[64] {
...
static const uint512_t C_TABLE[12] {
...
uint512_t operator ^(const uint512_t &lhs, const uint512_t &rhs)
{
	uint512_t result;
	for (unsigned i = 0; i < 8; ++i)
		result.v64[i] = lhs.v64[i] ^ rhs.v64[i];
	return result;
}
